...
- **SKILL.md 规则 10「砍评级悬念开头，技术文要有一手证据」**：两个词级检查抓不到的深层指纹，改写规则从九条扩到十条。
- **phrases.md 第 15 类「评级悬念词」**：AI 转述材料时先评级再陈述的前缀——"最反直觉的是 / 最容易踩坑的是 / 最值得注意的是 / 有意思的是 / 更微妙的是 / 反直觉"等。配额：全文"最 XX 的是"最多留 1 个。
- **structures.md 第 17 类「评级悬念开头」**：第 15 类的段落级形态——每个小节都用"先评级、再陈述"的模子起头。自检法：把所有小节的第一句抄到一起看，和第 16 类（标题套路）同理，整齐度藏在开头句的排列里。
- **structures.md 第 18 类「转述腔」**：技术文 / 源码解读 / 评测专属——号称深度实测却一行代码块、一条报错原文都没有，全是"如果你 X 会 Y"的假设推演，作者不在场。判断标准：读者能否找到至少一件"只有真读过的人才拿得出"的东西。
- **交付前清单新增 4 项**：搜"不是…而是…"超过 2 处就拆（structures.md 第 2 类早有此规则，但从未进过清单，等于形同虚设）；小节第一句抄到一起查评级悬念；搜"反直觉 / 出乎意料"评级词；技术文查一手证据。
- **SKILL.md 规则 2 补二元对立**："不是 X 而是 Y / 重要的不是 A / 与其 X 不如 Y"并入排比同族，正文规则层显式覆盖。

### 背景

- 真实案例：一篇 Wan2.2 源码解读通过了全部词级检查（破折号 0、无"首先其次"、无金句收尾），但仍一眼 AI 味。复盘发现三个漏网指纹：八个小节里六个用"最 XX 的是"起头；号称"读完全部源码"却全文没有一行代码块；图文排布每节固定"标题 → 引入 → 插图 → 两段分析"。词级 checklist 抓表层 token，这三个都藏在开头句排列、证据层和节奏里，故补规则 10 + 三个 references 条目。
