...
- **structures.md 第 16 类「标题套路」**：长文 / 技术文档专属指纹——正文干净但一排小标题用同一个模子（全"名词：点题"冒号副标题 / 全反问 / 全"为什么是 A 不是 B"对立）照样一眼 AI。给出"把标题抄到一起、超过一半同句式就打散"的自检法。SKILL.md 规则 8 与交付前清单同步加入。
- **触发机制优化**：SKILL.md frontmatter `description` / `trigger` 显式覆盖技术文档 / 技术方案 / 设计文档 / README / 博客 / 复盘 / 提案，并写明"发布类工具（如 blog-content-publish）不会自动串联本 Skill，需先手动改稿再发布"，提高写新稿/技术文档时的自动触发率。

### 背景

- 真实案例：一篇 chrome-use 技术博客正文相当干净（真实数字 + 诚实边界），但 14 个小标题里一半是"名词：点题"冒号副标题 + 3 个反问标题 + 1 个二元对立标题，目录扫一眼即露 AI 味。原 Skill 无标题层规则，故补第 16 类。
